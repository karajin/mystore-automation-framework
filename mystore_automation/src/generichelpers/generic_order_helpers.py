...
class GenericOrderHelpers:
    ...
    @staticmethod
    def verify_order_is_created(order, exp_cust_id, product_id):
        """

        :param order:
        :param exp_cust_id:
        :param product_id: list of dictionaries. Example: [{'product_id': product_id}]
        ...
        # verify response
        assert order, f"Create order response is empty."
        assert order['customer_id'] == exp_cust_id, f"Create order with given custoemr id returned" \
                                                         f"bad customer id.Expected customer_id={exp_cust_id} but got '{order['customer_id']}'"

        assert len(order['line_items']) == len(product_id), f"Expected only {len(product_id)} item in order but " \
                                                                   f"found '{len(order['line_items'])}'" \
                                                                   f"Order id: {order['id']}."
        ...
        assert len(line_items) == len(product_id), f"Expected {len(product_id)} line item but found {len(line_items)}. Order id: {order_id}"
    ...
    def list_all_orders(self, payload=None, expected_status_code=200):
        if not payload:
            payload={'per_page':100}
        elif 'per_page' not in payload.keys():
            payload['per_page']=100
        return self.order_api_helper.call_list_orders(payload=payload, expected_status_code=expected_status_code)